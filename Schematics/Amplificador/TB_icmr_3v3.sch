...
node="vout
vin"}
T {TB_ICMR_3v3 - Rango de modo comun de entrada (AMP V2)
Mismo seguidor que TB_offset_3v3; solo cambia el barrido: dc V2 -1.65..1.65
(Vin recorre TODO el riel 0..3.3V). ICMR = rango donde Vos se mantiene ~0.57mV.
Nota: en seguidor los extremos mezclan ICMR con la excursion de salida.} -740 -640 0 0 0.3 0.3 {}
...
let verr = abs(vosv)
echo ------------------------------------------------------------
echo LIMITES DE SEGUIMIENTO (donde el error entra/sale de la banda)
echo icmr_lo/hi_10mv: rango util con error menor a 10mV
echo icmr_lo/hi_1mv:  rango de precision con error menor a 1mV
echo ------------------------------------------------------------
* cross=2: el cruce 1 es trivial (vin~0 y vout~0 coinciden sin que el amp siga)
meas dc icmr_lo_10mv find v(vin) when verr=10m cross=2
meas dc icmr_hi_10mv find v(vin) when verr=10m cross=last
meas dc icmr_lo_1mv  find v(vin) when verr=1m  cross=1
meas dc icmr_hi_1mv  find v(vin) when verr=1m  cross=last
meas dc vos_cm165 find vosv when v(vin)=1.65
wrdata /foss/designs/CASS1/GF180_MIGRATED/V3P3/icmr_sweep_sch.txt v(vin) v(vout) vosv
plot v(vout) v(vin)
plot vosv ylimit -10m 10m
plot vosv
.endc
"}
...
C {BIAS/BIAS_3v3.sym} 140 -280 0 0 {name=x3}
